...
def main():
    ...
    y_reg_train = train_df['target_bw_300s_future']
    y_reg_val = val_df['target_bw_300s_future']
    y_class_train = train_df['target_is_off_spec_300s_future']
    y_class_val = val_df['target_is_off_spec_300s_future']
    
    cols_to_drop = ['target_bw_60s_future', 'target_bw_300s_future', 'target_is_off_spec_60s_future', 'target_is_off_spec_300s_future', 'event_id', 'split']
    
    # ABLATION: drop setpoints and targets
    ...
    # Drop any non-numeric (object) columns that LightGBM can't handle
    obj_cols = [c for c in train_df.columns if train_df[c].dtype == object]
    cols_to_drop.extend(obj_cols)
    if obj_cols:
        print(f"Dropping {len(obj_cols)} object columns: {obj_cols}")
    
    print(f"Dropping {len(ablation_cols)} setpoint columns: {ablation_cols}")
    
    # Only drop columns that actually exist in the dataframe
    cols_to_drop = [c for c in set(cols_to_drop) if c in train_df.columns]
    ...
    # Derive risk flag for regression using val_meta's active bw_setpoint since we dropped it
    pred_risk_val = (np.abs(pred_bw_val - val_df['bw_setpoint']) / val_df['bw_setpoint'] > 0.025).astype(int)
    ...
    # Build eval frames directly from the feature CSVs (same length as X_val/X_train)
    val_eval = pd.DataFrame({
        'event_id':  val_df['event_id'].values   if 'event_id'  in val_df.columns else val_meta['event_id'].values[:len(y_class_val)],
        'split':     'val',
        'y_true':    y_class_val.values,
        'pred_reg':  pred_risk_val,
        'pred_clf':  pred_class_val,
    })
    
    pred_bw_train    = model_reg.predict(X_train)
    pred_class_train = (model_clf.predict(X_train) > 0.5).astype(int)
    pred_risk_train  = (np.abs(pred_bw_train - train_df['bw_setpoint']) / train_df['bw_setpoint'] > 0.025).astype(int)
    
    train_eval = pd.DataFrame({
        'event_id': train_df['event_id'].values  if 'event_id' in train_df.columns else train_meta['event_id'].values[:len(y_class_train)],
        'split':    'train',
        'y_true':   y_class_train.values,
        'pred_reg': pred_risk_train,
        'pred_clf': pred_class_train,
    })
    
    all_meta = pd.concat([train_eval, val_eval], ignore_index=True)
    ...
    # 7. SAVE MODELS
    print("\nSaving ablated models...")
    joblib.dump(model_reg, './output/lgbm_regressor_ablated.pkl')
    joblib.dump(model_clf, './output/lgbm_classifier_ablated.pkl')
